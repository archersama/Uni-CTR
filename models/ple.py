...
class PLE(BaseModel):
    ...
    def __init__(self, feature_columns, sequence_feature_columns, scenarios, linear_feature_columns=[],
                 dnn_feature_columns=[],
                 input_dim=192,
                 shared_expert_num=1, specific_expert_num=1, num_levels=2,
                 expert_dnn_hidden_units=(512, 256, 128), gate_dnn_hidden_units=(128,), tower_dnn_hidden_units=(128,),
                 l2_reg_linear=0.00001, l2_reg_embedding=0.00001, l2_reg_dnn=0, init_std=0.0001, seed=1024,
                 dnn_dropout=0, dnn_activation='relu', dnn_use_bn=False, task_types=('binary', 'binary', 'binary'),
                 task_names=('ctr', 'ctcvr'), device='cpu', gpus=None):
        super(PLE, self).__init__(linear_feature_columns=[], dnn_feature_columns=[],
        ...
        self.num_scenario = len(self.scenarios)
    ...
    def forward(self, sparse_inputs, sequence_inputs, scenario_id):
        ...
        out = torch.zeros_like(task_outs[0])
        for i in range(self.num_scenario):
            out = torch.where(scenario_mask[i], task_outs[i], out)
        return out, task_outs