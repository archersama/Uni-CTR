...
def train_epoch(model, train_loader, optimizer, lr_scheduler, step, focalloss: bool, loss_fnc):
    ...
    for batch in tqdm_object:
        ...
        # Directly compute the output and loss without autocast
...
def main(model_name="ple"):
    ...
    # Create a list of parameters to be updated by the optimizer
    params_to_update = [param for name, param in model.named_parameters() if 'slot2_weight' not in name and 'slot2_bias' not in name]

    optimizer = torch.optim.Adam(params_to_update, lr=1, weight_decay=cfg.weight_decay, betas=(0.9, 0.999))
    ...
    for epoch in range(cfg.epochs):
        ...
        model.train()
        train_loss, train_auc = train_epoch(model, train_loader, optimizer, lr_scheduler, step, cfg.focal_loss, loss_fnc=loss_fn_focal if cfg.focal_loss else loss_fnc)
        lr_scheduler.step()
    ...
    # Create a list of parameters to be updated by the optimizer
    params_to_update = [param for name, param in model.named_parameters() if 'slot2_weight' in name or 'slot2_bias' in name]
